...
import { refreshSellerCreditScoreAction } from "@/modules/seller/sales/financing/actions";
...
/** Supplier copy of the buyer credit-score card, wired to the seller-side refresh action. */
export function SellerCreditScoreCard({ snapshot, locale }: { snapshot: ScoreSnapshot; locale: string }) {
  const t = useTranslations("financing.score");
  const { formAction } = useActionForm(refreshSellerCreditScoreAction);
