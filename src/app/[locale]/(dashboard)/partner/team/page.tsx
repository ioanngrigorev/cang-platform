...
import { getAuth, canCompany } from "@/modules/auth/current-user";
...
export default async function PartnerTeamPage({ params }: { params: Promise<{ locale: string }> }) {
  ...
  const { user, company } = await requirePartner({ permission: "company.profile.read" });
  const auth = await getAuth();
  const canManage = canCompany(auth, "company.members.manage");
  const t = await getTranslations("partner.team");
