...
import { buyerOrderIds } from "@/modules/payments/queries";
...
export default async function BuyerDocumentsPage({
  ...
  const t = await getTranslations("buyer.documents");

  const tab = sp.tab === "counterparty" ? "counterparty" : "ours";
  const orderIds = tab === "counterparty" ? await buyerOrderIds(company.id) : [];
  ...
  return (
    <>
      ...
      <LinkTabs
        ...
        className="mb-5"
        tabs={[
          { value: "ours", label: t("tabs.ours"), href: "/buyer/documents?tab=ours" },
          { value: "counterparty", label: t("tabs.counterparty"), href: "/buyer/documents?tab=counterparty" },
        ]}
      />
            ...
            {rows.map((d) => (
              <TR key={d.id}>
                ...
                <TD className="hidden text-steel-600 md:table-cell">
                  {d.order ? (
                    <Link href={`/buyer/orders/${d.order.id}`} className="hover:underline">
                      {d.order.orderNumber}
                    </Link>
                  ) : "rfq" in d && d.rfq ? (
                    <Link href={`/buyer/rfqs/${d.rfq.id}`} className="hover:underline">
                      {d.rfq.rfqNumber}
                    </Link>
                  ) : (
                    "—"
