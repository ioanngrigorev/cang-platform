...
export default async function SellerTeamPage({ params }: { params: Promise<{ locale: string }> }) {
  const { locale } = await params;
  const { user, company } = await requireCompany({ permission: "company.profile.read", seller: true });
  const auth = await getAuth();
  const canManage = canCompany(auth, "company.members.manage");
  const t = await getTranslations("seller.team");
