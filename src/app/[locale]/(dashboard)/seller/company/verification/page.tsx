...
export default async function SellerVerificationPage({ params }: { params: Promise<{ locale: string }> }) {
  const { locale } = await params;
  const { company } = await requireCompany({ permission: "company.verification.submit", seller: true });
  const t = await getTranslations("buyer.verification");
  const tc = await getTranslations("seller.company");
  ...
  return (
    <>
      <PageHeader
        ...
        breadcrumbs={[
          { label: tc("title"), href: "/seller/company" },
          { label: t("title") },
        ]}
      />
