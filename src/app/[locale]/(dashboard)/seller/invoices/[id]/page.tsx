...
import { getSellerInvoice } from "@/modules/seller/sales/invoices";

export const metadata: Metadata = { title: "Invoice", robots: { index: false } };

export default async function SellerInvoiceDetailPage({ params }: { params: Promise<{ locale: string; id: string }> }) {
  const { locale, id } = await params;
  const { company } = await requireCompany({ permission: "payments.read", seller: true });
  const t = await getTranslations("sales.invoices");

  const inv = await getSellerInvoice(company.id, id);
  if (!inv) notFound();
  ...
  return (
    <>
      <div className="print:hidden">
        <PageHeader
          breadcrumbs={[
            { label: t("title"), href: "/seller/invoices" },
      ...
      <Card className="print:border-0 print:shadow-none">
        <CardContent className="space-y-8 p-6 sm:p-8">
          <div className="flex flex-col gap-6 sm:flex-row sm:items-start sm:justify-between">
            <div>
              <p className="font-display text-2xl font-semibold text-ink-900">{t(`types.${inv.type}`)}</p>
              <p className="mt-1 text-sm text-steel-600">{inv.invoiceNumber}</p>
              {inv.order ? (
                <p className="mt-1 text-sm text-steel-600">
                  {t("orderRef")}:{" "}
                  <Link href={`/seller/orders/${inv.order.id}`} className="font-medium text-ink-900 hover:underline">
            ...
            <dl className="text-sm sm:text-right">
              ...
              {inv.paidAt ? (
                <div className="mt-1 flex gap-3 sm:justify-end">
                  <dt className="text-steel-500">{t("paidDate")}</dt>
                  <dd className="font-medium text-success-700">{formatDate(inv.paidAt, locale)}</dd>
                </div>
              ) : null}
            </dl>
          </div>
          ...
          <div className="grid gap-6 sm:grid-cols-2">
            <div>
              ...
              <p className="mt-1 font-medium text-ink-900">{inv.issuerCompany?.name ?? company.name}</p>
              {inv.issuerCompany?.address ? <p className="text-sm text-steel-600">{inv.issuerCompany.address}</p> : null}
              <p className="text-sm text-steel-600">{[inv.issuerCompany?.city ?? company.city, inv.issuerCompany?.countryCode ?? company.countryCode].filter(Boolean).join(", ")}</p>
              {inv.issuerCompany?.taxId ? (
                <p className="text-sm text-steel-600">
                  {t("taxId")}: {inv.issuerCompany.taxId}
                </p>
              ) : null}
            </div>
            <div>
              <p className="text-xs font-semibold uppercase tracking-wide text-steel-500">{t("billedTo")}</p>
              <p className="mt-1 font-medium text-ink-900">{inv.recipientCompany?.name ?? "—"}</p>
              {inv.recipientCompany?.address ? <p className="text-sm text-steel-600">{inv.recipientCompany.address}</p> : null}
              <p className="text-sm text-steel-600">{[inv.recipientCompany?.city, inv.recipientCompany?.countryCode].filter(Boolean).join(", ")}</p>
              {inv.recipientCompany?.taxId ? (
                <p className="text-sm text-steel-600">
                  {t("taxId")}: {inv.recipientCompany.taxId}
                </p>
              ) : null}
            </div>
          </div>
          ...
          {inv.order?.incoterm || inv.order?.paymentTerms ? (
            <div className="grid gap-4 border-y border-hairline py-4 text-sm sm:grid-cols-2">
              {inv.order?.incoterm ? (
                <p>
          ...
          {inv.payments.length ? (
            <div className="border-t border-hairline pt-4 print:hidden">
              <p className="text-xs font-semibold uppercase tracking-wide text-steel-500">{t("linkedPayments")}</p>
              <ul className="mt-2 space-y-1 text-sm">
                {inv.payments.map((p) => (
                  <li key={p.id} className="flex items-center justify-between gap-3">
                    <span className="text-ink-900">
                      {p.paymentNumber} · {p.milestoneLabel ?? humanize(p.kind)}
                    </span>
                    <span className="flex items-center gap-2">
                      <span className="tabular-nums text-steel-600">{formatMoney(p.amount, p.currency, locale)}</span>
