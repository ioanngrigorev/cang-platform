...
export default async function SellerDisputeDetailPage({ params }: { params: Promise<{ locale: string; id: string }> }) {
  const { locale, id } = await params;
  const { company } = await requireCompany({ permission: "disputes.manage", seller: true });
  const t = await getTranslations("sales.disputes");
  ...
  const counterparty = isRaiser ? d.respondentCompany : d.raisedByCompany;
  const awaitingUs = !isRaiser && d.status === "AWAITING_RESPONSE";

  return (
    <>
      <PageHeader
        breadcrumbs={[
          { label: t("title"), href: "/seller/disputes" },
        ...
        actions={
          <>
            <Button href={`/seller/orders/${d.order.id}`} variant="secondary">
              {d.order.orderNumber}
            </Button>
      ...
      {awaitingUs ? (
        <Alert variant="warning" title={t("respondTitle")} className="mb-6">
          {d.respondBy ? t("respondBody", { date: formatDate(d.respondBy, locale) }) : t("respondBodyNoDate")}
        </Alert>
      ) : null}
      {d.resolution ? (
      ...
      <div className="grid gap-6 lg:grid-cols-3 [&>*]:min-w-0">
        <Card className="lg:col-span-2">
          <CardHeader title={t("thread")} description={t("threadHint", { company: counterparty.name })} />
          <CardContent>
            <DisputeThread
              disputeId={d.id}
              locale={locale}
        ...
        <div className="space-y-6">
            ...
            <CardContent>
              <DataList
                ...
                items={[
                  ...
                  { label: t("raisedBy"), value: isRaiser ? t("you") : d.raisedByCompany.name },
                  { label: t("respondent"), value: isRaiser ? d.respondentCompany.name : t("you") },
                  ...
                  ...(d.resolvedAt ? [{ label: t("resolvedAt"), value: formatDate(d.resolvedAt, locale) }] : []),
                ]}
              />
              <div className="mt-4 border-t border-hairline pt-4">
                <p className="text-xs font-medium uppercase tracking-wide text-steel-500">{t("claim")}</p>
          ...
          <Card className="border-info-100 bg-info-50/40">
            <CardContent className="py-3 text-xs text-info-700">{t("mediationNote")}</CardContent>
          </Card>
        </div>
      </div>
    </>
