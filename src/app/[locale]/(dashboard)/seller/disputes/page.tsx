      ...
      {rows.length === 0 ? (
        ...
        <>
          <Table>
            ...
            <TBody>
              {rows.map(({ dispute: d, order }) => {
                const raisedByUs = d.raisedByCompanyId === company.id;
                const awaitingUs = !raisedByUs && d.status === "AWAITING_RESPONSE";
                return (
                  <TR key={d.id}>
                    <TD>
                      <Link href={`/seller/disputes/${d.id}`} className="font-medium text-ink-900 hover:underline">
                        {d.disputeNumber}
                      </Link>
                    ...
                    <TD className="hidden sm:table-cell">
                      ...
                      <p className="text-xs text-steel-500">{raisedByUs ? t("raisedByUs") : t("raisedByBuyer")}</p>
                    </TD>
                    ...
                    <TD className="hidden whitespace-nowrap text-steel-600 lg:table-cell">{d.respondBy ? formatDate(d.respondBy, locale) : "—"}</TD>
          ...
          <Pagination page={page} totalPages={totalPages} hrefFor={(p) => `/seller/disputes?page=${p}`} className="mt-6" />
        </>
      )}
    </>
  );
