...
export default async function SellerNotificationsPage({
  ...
  const t = await getTranslations("seller.notifications");
  ...
  return (
    <>
      ...
      <LinkTabs
        ...
        className="mb-5"
        tabs={[
          { value: "all", label: t("all"), href: "/seller/notifications?tab=all" },
          { value: "unread", label: t("unread"), href: "/seller/notifications?tab=unread", count: unread },
        ]}
      />
      ...
      {rows.length === 0 ? (
        ...
        <>
          ...
          <Pagination page={page} totalPages={totalPages} hrefFor={(p) => `/seller/notifications?tab=${tab}&page=${p}`} className="mt-6" />
        </>
      )}
    </>
  );
