...
export default async function SellerShipmentsPage({
  ...
  const [{ rows, totalPages }, shippable] = await Promise.all([listSellerShipments(company.id, { page }), sellerOrderOptions(company.id, SHIPPABLE_ORDER_STATUSES)]);
  const orderOptions = shippable.map((o) => ({ id: o.id, orderNumber: o.orderNumber, buyerName: o.buyerName }));

  return (
    <>
      <PageHeader title={t("title")} description={t("description")} actions={<CreateShipmentButton orders={orderOptions} />} />
